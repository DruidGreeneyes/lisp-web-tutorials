...
;;; "restas-test" goes here. Hacks and glory await!

(define-route home ("") 
  (list :title "Blogdemo" :body (render-posts *posts*)))
...
(define-route author ("author/:id")
  ...
  (let ((posts (loop for post in *posts*
                  if (string= id (getf post :author-id))
                     collect post)))
    (list :title (format nil "Posts by ~a" (getf (forst posts) :author))
          :body (render-posts posts))))
