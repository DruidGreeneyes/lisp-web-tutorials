...
(defparameter *db* (make-instance 'pg-datastore
                                  :connection-spec '("testdb" "restas" "restas" "localhost")))
  ...
  (with-connection (connection-spec datastore)
    (unless (datastore-find-user datastore username)
      (let ((password-salt (hash-password password)))
        (when (save-dao (make-instance 'users :name username
...
(defmethod datastore-upvoted-p ((datastore pg-datastore) link-id user)
  (with-connection (connection-spec datastore)
    (query (:select :link-id :user.name :from 'votes 'users
...
(defmethod datastore-upvote ((datastore pg-datastore) link-id user)
  (with-connection (connection-spec datastore)
    (let ((submitter-id (getf (datastore-find-user datastore user) :id)))
      (when (and submitter-id (not (datastore-upvoted-p datastore link-id user)))
...
(defmethod datastore-post-link ((datastore pg-datastore) url title user)
  (with-connection (connection-spec datastore)
    (let ((link (make-instance 'links :url url
                                      :title title
                                      :submitter-id (getf (datastore-find-user datastore user) :id))))
      (save-dao link)
...
(defun sort-links (links)
  (sort links #'> :key #'(lambda (link) (getf link :votes))))
  
(defmethod datastore-get-all-links ((datastore pg-datastore) &optional username)
  (with-connection (connection-spec datastore)
    (sort-links (add-vote-count datastore (get-all-links/internal) (or username "")))))
